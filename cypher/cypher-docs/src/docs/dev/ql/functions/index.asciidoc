...
The <<query-load-csv,`LOAD CSV`>> clause in Cypher will import data from a CSV file and run a Cypher statement on each row of that file.
If data on one row can lead to an error in the query execution, that error will <<load-csv-import-data-with-errors,include the filename and linenumber>> of the original CSV file to aid in fixing the import data.
This information is also available to Cypher statements using the following functions.

[options="header"]
|===
|Function                              | Description
|<<functions-filename,filename()>>     | Returns the current file name of the CSV file being imported, or null if we are not in `LOAD CSV`.
|<<functions-linenumber,linenumber()>> | Returns the current line number of the CSV file being imported, or null if we are not in `LOAD CSV`.
|===
